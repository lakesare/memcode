...
to_char(timezone('+05:00'::text, "user".created_at), 'YYYY DD Month, HH24:MI'::text) AS user_created_at,
course.title,
course.if_public,
course.id,
to_char(timezone('+05:00'::text, course_user_is_learning.started_learning_at), 'DD Month, HH24:MI'::text) AS started_learning_at
...
SELECT "user".username,
   ...
   course.if_public,
   problem.content,
   to_char(timezone('+05:00'::text, problem_user_is_learning.last_reviewed_at), 'DD Month, HH24:MI'::text) AS last_reviewed_at
