...
CREATE VIEW "timeline_of_joined_courses" AS
SELECT "user".username,
to_char("user".created_at, 'DD Month, HH24:MI'::text) AS user_created_at,
course.title,
course.if_public,
course.id,
...
CREATE VIEW "timeline_of_reviewed_flashcards" AS
SELECT "user".username,
   to_char(timezone('YEKT'::text, timezone('UTC'::text, "user".created_at)), 'YYYY DD Month, HH24:MI'::text) AS user_created_at,
   course.title AS course_title,
   course.id AS course_id,
   course.if_public,
   problem.content,
   to_char(timezone('+03:00'::text, timezone('UTC'::text, problem_user_is_learning.last_reviewed_at)), 'DD Month, HH24:MI'::text) AS last_reviewed_at
